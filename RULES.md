...
### `verbatim` — Программируемые объекты выводятся дословно
`CREATE`/`ALTER [OR ALTER]` `FUNCTION` / `PROCEDURE` / `PROC` / `TRIGGER` содержат
процедурное тело (`begin/end`, `if`, `while`, `set`, `return`), которое форматтер не
переформатирует. Такой объект захватывается **дословно** (все токены, включая пробелы и
комментарии) и выводится без изменений — до батч-разделителя `GO` или конца ввода.
Это гарантирует сохранность и синтаксиса, и семантики. Парсер также защищён от зацикливания:
любой цикл разбора гарантированно продвигается вперёд (не «зависает» на неожиданном токене).
