...
### `4.1` — Ведущий блочный `/* */` перед колонкой
Блочный комментарий перед колонкой сохраняется на той же строке перед выражением. В IN-списках значения группируются по общему комментарию.
...
### `2.6` — Одно условие inline; несколько — по строкам, and/or в начале
...
### `2.15` / `declare` — Одна переменная inline; несколько — по строкам
Типы: `varchar(50)` без внутренних пробелов, `decimal(18, 2)` — пробел после запятой. Хвостовой комментарий переменной сохраняется.
