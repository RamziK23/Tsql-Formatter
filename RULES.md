...
- Правил: ~80 · Тестов: 164/164 · Движок: .NET 5
...
`from openquery(` без продолжения), всё, что успело разобраться до неё, форматируется, а
незавершённый хвост выводится дословно. Раньше из-за такого хвоста не форматировался весь скрипт.
...
select object_type from openquery(
-- результат
create table #process (
	object_type varchar(255),
	object_id int
