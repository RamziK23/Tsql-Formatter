...
без изменений**, а не выдаёт частично-переформатированный (битый) результат. Также `--`
...
Список колонок: открывающая `(` вплотную к имени таблицы, **каждая колонка на своей строке** (+1 таб), закрывающая `)` на отдельной строке на уровне `insert`. Работает и когда список колонок — единственное, что есть в выделении (без `values`/`select`).
